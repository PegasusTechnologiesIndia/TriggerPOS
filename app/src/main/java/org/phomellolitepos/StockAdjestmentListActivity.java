...
public class StockAdjestmentListActivity extends AppCompatActivity {
    EditText edt_toolbar_item_list;
    TextView item_title;
    ArrayList<Stock_Adjustment_Header> arrayList;
    Stock_Adjustment_Header stock_adjustment_header;
    StockAdjestmentListAdapter stockAdjestmentListAdapter;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_stock_adjestment_list);
        ...
        toolbar.setNavigationOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                pDialog = new ProgressDialog(StockAdjestmentListActivity.this);
                ...
                Thread timerThread = new Thread() {
                    public void run() {
                        if (settings.get_Home_Layout().equals("0")) {
                            try {
                                Intent intent = new Intent(StockAdjestmentListActivity.this, MainActivity.class);
                            ...
                            try {
                                Intent intent = new Intent(StockAdjestmentListActivity.this, Main2Activity.class);
                                intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK);
                                startActivity(intent);
                                pDialog.dismiss();
                                finish();
        ...
        fab.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                Intent intent = new Intent(StockAdjestmentListActivity.this, StockAdjestmentHeaderActivity.class);
    ...
    private void getStockList(String strFilter) {
        arrayList = Stock_Adjustment_Header.getAllstock_adjustment_header(getApplicationContext(), "WHERE is_active = '1' " + strFilter + " Order By lower(date) asc", database);
        ListView category_list = (ListView) findViewById(R.id.item_list);
        if (arrayList.size() > 0) {
            stockAdjestmentListAdapter = new StockAdjestmentListAdapter(StockAdjestmentListActivity.this, arrayList);
            category_list.setVisibility(View.VISIBLE);
            item_title.setVisibility(View.GONE);
            category_list.setAdapter(stockAdjestmentListAdapter);
            category_list.setTextFilterEnabled(true);
            stockAdjestmentListAdapter.notifyDataSetChanged();
        } else {
    ...
    private void send_email_manager(String strEmail, String strFileName, String strReportName) {
        ...
        try {
            ...
            final StockAdjestmentListActivity.SendEmailAsyncTask email = new StockAdjestmentListActivity.SendEmailAsyncTask();
            email.activity = this;
    ...
    class SendEmailAsyncTask extends AsyncTask<Void, Void, Boolean> {
        GMailSender m;
        StockAdjestmentListActivity activity;
...
//                activity.displayMessage("Unexpected error occured.");
    ...
    protected void performPDFExport(String voucher_no, String s,String name) {
        ...
        ArrayList<Stock_Adjustment_Detail> stock_adjustment_detailArrayList = Stock_Adjustment_Detail.getAllstock_adjustment_detail(getApplicationContext(), "where ref_voucher_no ='" + voucher_no + "'", database);
        int count = 0;
        Double total_qty = 0d;
        Double total_price = 0d;
        Double total_amount = 0d;
        while (count < stock_adjustment_detailArrayList.size()) {
            total_qty = total_qty + Double.parseDouble(stock_adjustment_detailArrayList.get(count).get_qty());
//            total_price = total_price + Double.parseDouble(stock_adjustment_detailArrayList.get(count).get_price());
//            total_amount = total_amount + Double.parseDouble(stock_adjustment_detailArrayList.get(count).get_line_total());

            String item_code = stock_adjustment_detailArrayList.get(count).get_item_code();
            ...
            list4a.add(stock_adjustment_detailArrayList.get(count).get_qty());
            list6a.add(stock_adjustment_detailArrayList.get(count).get_in_out_flag());
//            list7a.add(stock_adjustment_detailArrayList.get(count).get_line_total());
        ...
        try {
            ...
            PdfPCell cellh = new PdfPCell(new Paragraph("Stock Adjustment", B12));
            cellh.setColspan(1);
            ...
            pr = new Phrase("" + stock_adjustment_header.get_voucher_no(), B10);
            ...
            pr = new Phrase("" + stock_adjustment_header.get_date().trim().substring(0, 10), B10);
...
//            table_cash_total.setSpacingBefore(10.0f);
            ...
            pr21 = new Phrase("" + stock_adjustment_header.get_remarks(), B10);
...
//            pr23 = new Phrase("" + stock_adjustment_header.get_contact_code(), B10);
//
