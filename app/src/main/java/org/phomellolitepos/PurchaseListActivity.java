...
public class PurchaseListActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        toolbar.setNavigationOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                pDialog = new ProgressDialog(PurchaseListActivity.this);
                ...
                Thread timerThread = new Thread() {
                    public void run() {
                        if (settings.get_Home_Layout().equals("0")) {
                            ...
                            try {
                                Intent intent = new Intent(PurchaseListActivity.this, Main2Activity.class);
                                intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TASK);
                                startActivity(intent);
                                pDialog.dismiss();
                                finish();
        ...
        fab.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                Intent intent = new Intent(PurchaseListActivity.this, PurchaseHeaderActivity.class);
    ...
    private void getStockList(String strFilter) {
        arrayList = Purchase.getAllPurchase(getApplicationContext(), "WHERE is_active = '1' " + strFilter + " Order By lower(date) asc", database);
        ListView category_list = (ListView) findViewById(R.id.item_list);
        if (arrayList.size() > 0) {
            purchaseListAdapter = new PurchaseListAdapter(PurchaseListActivity.this, arrayList);
            category_list.setVisibility(View.VISIBLE);
            item_title.setVisibility(View.GONE);
            category_list.setAdapter(purchaseListAdapter);
    ...
    private void send_email_manager(String strEmail, String strFileName, String strReportName) {
        ...
        try {
            ...
            final PurchaseListActivity.SendEmailAsyncTask email = new PurchaseListActivity.SendEmailAsyncTask();
            email.activity = this;
...
//                activity.displayMessage("Unexpected error occured.");
    ...
    protected void performPDFExport(String voucher_no, String s,String name) {
        ...
        ArrayList<Purchase_Detail> purchase_detailArrayList = Purchase_Detail.getAllPurchase_Detail(getApplicationContext(), "where ref_voucher_no ='" + voucher_no + "'", database);
        int count = 0;
        Double total_qty = 0d;
        Double total_price = 0d;
        Double total_amount = 0d;
        while (count < purchase_detailArrayList.size()) {
            total_qty = total_qty + Double.parseDouble(purchase_detailArrayList.get(count).get_qty());
            total_price = total_price + Double.parseDouble(purchase_detailArrayList.get(count).get_price());
            total_amount = total_amount + Double.parseDouble(purchase_detailArrayList.get(count).get_line_total());

            String item_code = purchase_detailArrayList.get(count).get_item_code();
            ...
            list4a.add(purchase_detailArrayList.get(count).get_qty());
            list6a.add(purchase_detailArrayList.get(count).get_price());
            list7a.add(purchase_detailArrayList.get(count).get_line_total());

            count++;
        }

        ArrayList<Purchase_Payment> purchase_paymentArrayList = Purchase_Payment.getAllPurchase_Payment(getApplicationContext(), "where ref_voucher_no ='" + voucher_no + "'");
        int count1 = 0;
        Double total_payment = 0d;

        while (count1 < purchase_paymentArrayList.size()) {

            total_payment = total_payment + Double.parseDouble(purchase_paymentArrayList.get(count1).get_pay_amount());

            Payment payment = Payment.getPayment(getApplicationContext(), "WHERE payment_id = '" + purchase_paymentArrayList.get(count1).get_payment_id() + "'");
            if (payment != null) {
                list8a.add(payment.get_payment_name());
            }
            list9a.add(purchase_paymentArrayList.get(count1).get_pay_amount());
            count1++;
        }
        ...
        try {
            ...
            PdfPCell cellh = new PdfPCell(new Paragraph("Purchase", B12));
            cellh.setColspan(1);
            ...
            pr = new Phrase("" + purchase.get_voucher_no(), B10);
            ...
            pr = new Phrase("" + purchase.get_date().trim().substring(0, 10), B10);
...
//            table_cash_total.setSpacingBefore(10.0f);
            ...
            pr23 = new Phrase("" + purchase.get_contact_code(), B10);
...
//            PdfPTable supplier_name = new PdfPTable(2);
            ...
            if (contact != null) {
                pr24 = new Phrase("" + purchase.get_contact_code(), B10);
...
//            table_total.setSpacingBefore(10.0f);
            ...
            for (int i = 0; i < list8a.size(); i++) {
                ...
                pr = new Phrase(Globals.myNumberFormat2Price(Double.parseDouble(list9a.get(i)), decimal_check), N9);
                c7 = new PdfPCell(pr);
                c7.setPadding(5);
                c7.setHorizontalAlignment(Element.ALIGN_LEFT);
                table_payment.addCell(c7);
